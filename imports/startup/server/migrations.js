...
Migrations.add({
    version: 12,
    name: 'update userId from emails " ',
    up: function () {
        let no = 0, si = 0
        Email.find({$or: [{userId: {$exists: 0}}, {userId: ''}]}).forEach((email) => {
            const family = Meteor.users.findOne({$or: [{"emails.address": email.to}, {"parents.email": email.to}]})
            if (family) {
                si++
                Email.update(email._id, {$set: {userId: family._id}})
            } else {
                no++
            }
